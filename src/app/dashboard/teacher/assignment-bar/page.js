...
export default function TeacherDashboard() {
  ...
  useEffect(() => {
    const fetchCourseDetails = async () => {
      ...
      const combinedSubmissions = allCourseDetails.reduce(
        (acc, courseData) => acc + courseData.totalSubmissions,
        0
      );
      const combinedStudents = allCourseDetails.reduce(
        (acc, courseData) => acc + courseData.totalStudents,
        0
      );
